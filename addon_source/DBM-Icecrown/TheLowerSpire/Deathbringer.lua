...
local timerCombatStart        = mod:NewTimer(46, "TimerCombatStart", 2457)
...
function mod:OnCombatStart(delay)
    ...
    if mod:IsInRaidDifficulty("normal10", "normal25") then
...
do
    ...
    mod:RegisterOnUpdateHandler(function(self)
        if self.Options.BeastIcons and (DBM:GetRaidRank() > 0 and not (iconsSet == 5 and self:IsInRaidDifficulty("normal25", "heroic25") or iconsSet == 2 and self:IsInRaidDifficulty("normal10", "heroic10"))) then
...
function mod:SPELL_AURA_APPLIED(args)
    ...
    elseif args:IsSpellID(72385, 72441, 72442, 72443) then    -- Boiling Blood
        ...
        if mod:IsInRaidDifficulty("normal10", "heroic10") or (mod:IsInRaidDifficulty("normal25", "heroic25") and #boilingBloodTargets >= 3) then    -- Boiling Blood

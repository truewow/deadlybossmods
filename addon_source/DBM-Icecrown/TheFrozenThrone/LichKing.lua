...
local timerCombatStart        = mod:NewTimer(70, "TimerCombatStart", 2457)
...
do
    ...
    mod:RegisterOnUpdateHandler(function(self)
        if self.Options.ValkyrIcon and (DBM:GetRaidRank() > 0 and not (iconsSet == 3 and self:IsInRaidDifficulty("normal25", "heroic25") or iconsSet == 1 and self:IsInRaidDifficulty("normal10", "heroic10"))) then
...
function mod:UNIT_HEALTH(uId)
    if mod:IsInRaidDifficulty("heroic10", "heroic25") and uId == "target" and self:GetUnitCreatureId(uId) == 36609 and UnitHealth(uId) / UnitHealthMax(uId) <= 0.55 and not warnedValkyrGUIDs[UnitGUID(uId)] then

...
function mod:SPELL_AURA_APPLIED(args)
    if args:IsSpellID(69279) then    -- Gas Spore
        ...
        if (gasSporeCast < 9 and mod:IsInRaidDifficulty("normal25", "heroic25")) or (gasSporeCast < 6 and mod:IsInRaidDifficulty("normal10", "heroic10")) then
            timerGasSporeCD:Start()
            timerVileGas:Start(31)
        elseif (gasSporeCast >= 9 and mod:IsInRaidDifficulty("normal25", "heroic25")) or (gasSporeCast >= 6 and mod:IsInRaidDifficulty("normal10", "heroic10")) then
        ...
        if self.Options.SetIconOnGasSpore then
            ...
            if (mod:IsInRaidDifficulty("normal25", "heroic25") and #gasSporeIconTargets >= 3) or (mod:IsInRaidDifficulty("normal10", "heroic10") and #gasSporeIconTargets >= 2) then
